...
| 11 | **A print treatment renders as a blank page** | Newsprint/riso/screen-print styles will happily produce columns of type texture and nothing else, which reads as empty. **Every print treatment must carry a picture** — name a photo block in the right third, in a thin keyline box. That photo plus the 2–4 reason lines under it is the entire content; do NOT add headings or page furniture to compensate (see 15, 15b). If it still reads thin, add another VK-backed reason line and shrink the photo — never add furniture |
...
| 15b | **The block under the photo — the only body text allowed** | Not a decorative caption: it explains **WHY the hero commodity moved**. Set it as **ONE CONTINUOUS small justified paragraph of body copy running about four lines** — tight leading, no dividing rules, no bullets, no separate boxes. It must read as flowing newspaper column text; **four separately-boxed lines read as a list, not a newspaper.** Every sentence is lifted from that day's VK paper — real reported fact, never invented, never generic — ordered cause first, consequence last. Worked example for a sugar crash: `केंद्र ने अगस्त का 22.50 लाख टन कोटा जारी किया। मिलों को पूरी बिक्री और उठाव अनिवार्य। न बेचने पर सख्त कार्रवाई की चेतावनी। दिल्ली में खुदरा भाव 68 से घटकर 62 रुपये।` **Shrink the photo to make room.** Never repeat a number the headline or hero line already shows. If the paper supports only two honest sentences, use two — never pad |
| 17 | **Readable English on objects inside the photo** | A sugar sack rendered with `SUGAR` stencilled on it — the no-English rule applies to the photograph too, not just the overlay. Every prompt must carry an explicit clause on the depicted objects: **plain and unmarked, no printed words, brand names, labels or stencilling anywhere in the photo.** This clause is load-bearing; it was once dropped while trimming for the character budget and the English came straight back |
| 18 | **Print treatments render on flat digital white** | A modern/clean print style loses the paper entirely and looks like a slide, not a page. State the stock explicitly for EVERY print treatment, modern ones included: **warm off-white newsprint with visible grain and mottling, NOT flat white** |
